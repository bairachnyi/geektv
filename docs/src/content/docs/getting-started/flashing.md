---
title: Flashing
description: How to install SmallTV Ultra firmware on each board, back up the stock firmware first, and recover if something goes wrong.
---
...
3. Upload `smalltv-ultra-firmware.bin`. It reboots into this firmware.
...
esptool.py --port COM5 write_flash 0x0 smalltv-ultra-firmware.bin
```
...
The SmallTV-ultra is the same ESP-12F (ESP8266) as the original SmallTV: same 4 MB flash, the same 1.54" 240×240 ST7789 panel, and the same pin map. Only its stock firmware differs. The "Ultra" firmware (for example Ultra-V9.0.50) reserves most of the flash for image and GIF storage, which leaves its OTA updater a small app slot. Uploading `smalltv-ultra-firmware.bin` at `/update` can fail with `Update error: ERROR[4]: Not Enough Space`, even though the flash has free user space, because the full image does not fit that small slot.
...
5. It comes up in the usual `SmallTV-Setup` captive portal for WiFi. From here it is a normal ESP8266 SmallTV Ultra device.
...
If the loader ever will not come back up, flash over the serial header instead. The board is a plain ESP-12F, so the [UART recovery](#uart-header-for-recovery) steps above apply directly: `esptool.py --port COM5 write_flash 0x0 smalltv-ultra-firmware.bin`.
...
```bash
python -m esptool --chip esp32 --port COM3 --baud 921600 write_flash 0x0 smalltv-ultra-firmware-esp32.factory.bin
```
