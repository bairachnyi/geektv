...
Credentials are compile-time only and never live in the repo. The build output is published as the release asset `smalltv-ultra-loader.bin`.
...
- The four feature modes, the web UI, and the settings layer are identical across all targets.
...
src/                    shared core (device, net, web, settings)
  ...
  OtaUpdate.*           GitHub self-update
  features/
    ticker/             TickerMode + StockClient
    usage/              UsageMode + UsageClient + Mascot
    github/             GH//STAT mode + compact bridge client
emulator/               GitHub bridge and browser emulator
...
The ESP8266 v0.3 build is about 704 KB of flash and about 60 percent of static RAM, with headroom for OTA, which needs room for two sketch copies. Exact values are printed by every PlatformIO build and can change with features. The mascot frame data and embedded web UI live in flash, not the heap.

The AI usage screen accepts a compact JSON feed from a trusted LAN bridge.
