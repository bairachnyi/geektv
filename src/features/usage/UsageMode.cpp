...
// One usage card: big %, a provider label, a fill bar coloured by load, and the
...
// Stats screen: mascot header + Antigravity/Codex meters.
static void drawUsage(const UsageData& u) {
  ...
  if (!u.valid) {
    gfxDrawCentered(u.error ? "bridge error" : "waiting...", 120, 2, C_DIM);
    return;
  }

  // Partial/error data gets a small accent flag.
  if (u.status[0] && strcmp(u.status, "ok") != 0 && strncmp(u.status, "allowed", 7) != 0) {
    gfx->fillCircle(228, 18, 5, C_ACCENT);
  }

  drawMeter(gfx, 50,  "ANTIGRAVITY", u.antigravityPct, u.antigravityResetMin);
  drawMeter(gfx, 138, "CODEX",       u.codexPct,       u.codexResetMin);
}
...
void UsageMode::service(const Settings& s) {
  // Pull mode: poll the bridge when a URL is set. Push mode: leave it blank
  // and the bridge POSTs to /api/ai-usage (for networks where the device can't reach
  ...
  if (u.valid && u.lastOkMs != usageSampled_) {
    ...
    mascotSample(max(u.antigravityPct, u.codexPct));
  }
