...
void clockReapply(const Settings& s) {
  // SNTP is needed for clock display regardless of night mode. The heap cost
  // is acceptable — the clock cannot show time without it.
...
String clockTimeStr() {
  ...
  snprintf(b, sizeof(b), "%04u-%02u-%02u %02u:%02u",
           ...
           (unsigned)t.tm_mday, (unsigned)t.tm_hour, (unsigned)t.tm_min);
  return String(b);
}
