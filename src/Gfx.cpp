...
void gfxDrawRect(int x, int y, int w, int h, uint16_t color) {
  if (gfx) gfx->drawRect(x, y, w, h, color);
}

void gfxFillRoundRect(int x, int y, int w, int h, int r, uint16_t color) {
  if (gfx) gfx->fillRoundRect(x, y, w, h, r, color);
}

void gfxDrawRoundRect(int x, int y, int w, int h, int r, uint16_t color) {
...
void gfxHeader(const char* title, const char* rightText) {
  ...
  if (rightText && rightText[0]) {
    int w = (int)strlen(rightText) * 6;
    gfx->setTextColor(0xFFFF);
    gfx->setCursor(236 - w, 4);
    gfx->print(rightText);
  }
  gfx->drawFastHLine(0, 16, 240, 0x1930);
}
