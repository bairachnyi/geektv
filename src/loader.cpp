...
// reserves so much flash that its updater rejects a full smalltv-ultra image
...
// the stock OTA. Once running it uses smalltv-ultra's normal 4m1m flash layout,
...
void setup() {
  ...
  if (!joined) {
    WiFi.mode(WIFI_AP);
    WiFi.softAP("GeekTV-Loader");        // open AP -> browse to 192.168.4.1/update
  }
